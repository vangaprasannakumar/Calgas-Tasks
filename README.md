...
- **Assignment emails go out individually, not as one email to everyone.** When a task is created, every Responsible and Monitoring person gets their own personalized email (their own name, their own role label) rather than all being listed in one email's To: field — so nobody sees everyone else's address, and someone who's in both roles gets a single combined email rather than two. Sent after the sheet writes complete and the lock is released, so slow email I/O never extends how long a task-creation request holds the lock; if a notification fails for one recipient (bad address, quota, etc.) it doesn't stop the others from going out or fail the task creation itself.
- **The email's "Open task →" button deep-links** via `?task=CT-00001` in the URL — the app checks for that on login/session-restore and opens the task detail directly, then cleans the URL so a later refresh doesn't reopen it.
...
- **No email notifications on status changes or comments** (e.g. "your task was marked Completed") — only task *creation* sends an email right now. Everything else is still pull-based. Could be added as a follow-up if useful.
